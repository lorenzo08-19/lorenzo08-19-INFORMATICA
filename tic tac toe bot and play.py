...
def controlla_vittoria(tabellone, simbolo):
    for i in range(3):
        if all(tabellone[i][j] == simbolo for j in range(3)):
            return True
    for i in range(3):
...
def mosse_possibili(tabellone):
    return [(i, j) for i in range(3) for j in range(3) if tabellone[i][j] == ' ']
...
def esegui_turno_bot(tabellone, giocatore):
    print(f"\nTurno del bot: {giocatore['nome']}")
    simbolo = giocatore['simbolo']
    mosse = mosse_possibili(tabellone)

    # 1. Cerca una mossa vincente
    for i, j in mosse:
        copia = [riga[:] for riga in tabellone]
        copia[i][j] = simbolo
        if controlla_vittoria(copia, simbolo):
            tabellone[i][j] = simbolo
            return

    # 2. Altrimenti, gioca una mossa casuale
    mossa = random.choice(mosse)
    tabellone[mossa[0]][mossa[1]] = simbolo

def gioco_tic_tac_toe():
    giocatore1 = {'nome': input("Nome del primo giocatore: "), 'simbolo': 'X', 'vittorie': 0}
    giocatore2 = {'nome': 'Bot', 'simbolo': 'O', 'vittorie': 0, 'bot': True}
    ...
    while not partita_completata:
        ...
        while True:
            ...
            giocatore_attuale = giocatori[turno % 2]
            if 'bot' in giocatore_attuale:
                esegui_turno_bot(tabellone, giocatore_attuale)
            else:
                esegui_turno(tabellone, giocatore_attuale)

            if controlla_vittoria(tabellone, giocatore_attuale['simbolo']):
                stampa_tabellone(tabellone)
                print(f"\n{giocatore_attuale['nome']} ha vinto!")
                giocatore_attuale['vittorie'] += 1
                break
